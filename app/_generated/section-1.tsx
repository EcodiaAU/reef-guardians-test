...
export default function Story(props: StoryProps): ReactElement {
  const { heading, paragraphs, quote, attribution } = props;
  const t = useTheme(props.theme);
  return (
    <section data-eos-id="app/_generated/section-1.tsx#0" data-eos-v="2" className="px-6 py-20 sm:py-28" style={{ backgroundColor: t.surfaceAlt, color: t.ink }}>
      <div data-eos-id="app/_generated/section-1.tsx#1" style={container(t)}>
        <div data-eos-id="app/_generated/section-1.tsx#2" className="mx-auto" style={{ maxWidth: '42rem' }}>
        <h2 data-eos-id="app/_generated/section-1.tsx#3" className="text-3xl font-semibold tracking-tight sm:text-4xl">{heading}</h2>
        <div data-eos-id="app/_generated/section-1.tsx#4" className="mt-6 space-y-5 text-lg leading-8" style={{ color: t.muted }}>
          {paragraphs.map((p, i) => (
            <p data-eos-id="app/_generated/section-1.tsx#5" key={i}>{p}</p>
          ))}
        </div>
        {quote ? (
          <figure data-eos-id="app/_generated/section-1.tsx#6" className="mt-10 pl-6" style={{ borderLeft: '3px solid ' + t.primary }}>
            <blockquote data-eos-id="app/_generated/section-1.tsx#7" className="text-xl font-medium leading-9" style={{ color: t.ink }}>
              {quote}
            </blockquote>
            {attribution ? (
              <figcaption data-eos-id="app/_generated/section-1.tsx#8" className="mt-3 text-sm" style={{ color: t.muted }}>
                {attribution}
              </figcaption>
            ) : null}
          </figure>
