...
export default function Services(props: ServicesProps): ReactElement {
  ...
  return (
    <section data-eos-id="app/_generated/section-2.tsx#0" data-eos-v="2" className="px-6 py-20 sm:py-28" style={{ backgroundColor: t.surface, color: t.ink }}>
      <div data-eos-id="app/_generated/section-2.tsx#1" style={container(t)}>
        <h2 data-eos-id="app/_generated/section-2.tsx#2" className="text-3xl font-semibold tracking-tight sm:text-4xl">{heading}</h2>
        {subtitle ? (
          <p data-eos-id="app/_generated/section-2.tsx#3" className="mt-4 text-lg leading-8" style={{ maxWidth: '40rem', color: t.muted }}>
            {subtitle}
          </p>
        ) : null}
        <div data-eos-id="app/_generated/section-2.tsx#4" className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((s) => (
            <div data-eos-id="app/_generated/section-2.tsx#5"
              key={s.title}
              className="p-7"
              style={{ backgroundColor: t.surfaceAlt, border: '1px solid ' + t.line, borderRadius: t.radius }}
            >
              <h3 data-eos-id="app/_generated/section-2.tsx#6" data-eos-var="s.title" data-eos-var-label="Title" data-eos-var-scope="item" className="text-lg font-semibold">{s.title}</h3>
              <p data-eos-id="app/_generated/section-2.tsx#7" data-eos-var="s.description" data-eos-var-label="Description" data-eos-var-scope="item" className="mt-3 text-sm leading-7" style={{ color: t.muted }}>
                {s.description}
              </p>
            </div>
          ))}
